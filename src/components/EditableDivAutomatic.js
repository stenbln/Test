...
import * as SortableActionsAutomatic from '../actions/SortableActionsAutomatic'
  ...
  var EditableDivAutomatic = React.createClass({
    referenceElement(dataitemID){
      return document.getElementById("sortableDivAutomatic").childNodes[0].childNodes[dataitemID].childNodes[0].childNodes[0];
    },
    ...
    changeParticularItem(evt){
      ...
      if(evt.type!='blur'){
        ...
        if(tmp!=""){
          SortableActionsAutomatic.changeSortableItem(tmp,this.props.dataitemID);
        }
        //console.log("ovo je evt.key item", evt);
    ...
    if (evt.key === 'Enter'){
        ...
        if(caretPos!=0){
          ...
          SortableActionsAutomatic.addNewItem(textContent,this.props.dataitemID);
        }
      }
    },
    ...
    deleteItem(evt,caretPos){
      if(evt.keyCode == 46 || evt.keyCode == 8){
        ...
        SortableActionsAutomatic.deleteItem(this.props.dataitemID,caretPos,fieldValueAfterPressingDelete);
      }
    },
    ...
    handleBlur(){//not implemented
      if(this.referenceElement(this.props.dataitemID).textContent==""){
        ...
        SortableActionsAutomatic.deleteItem(this.props.dataitemID);
      }
    },
    render: function(){
  ...
  module.exports = EditableDivAutomatic;